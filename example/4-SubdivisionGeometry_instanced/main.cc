...
void glfwKeyCallback(GLFWwindow *window, int key, int scancode, int action,
    ...
    if (action == GLFW_PRESS) {
        ...
        case GLFW_KEY_PERIOD:
            ++subdivision;
            subdivision = std::max(1, std::min(8, subdivision));
            break;
        case GLFW_KEY_COMMA:
            --subdivision;
            subdivision = std::max(1, std::min(8, subdivision));
...
void init() {
    ...
    assert(shader.createProgram("SubdivisionGeometry_instanced.vert",
                                "SubdivisionGeometry_instanced.frag",
                                "SubdivisionGeometry_instanced.geom"));
